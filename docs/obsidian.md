...
1. **記録** — チャットの内容をフックが vault に書き残す（会話 → vault）
2. **呼び出し** — 質問に関連するノートをフックが会話に差し込む（vault → 会話）
3. **再開** — 前回までのやり取りをセッション開始時に差し込む（vault → 会話）
4. **読み書き** — Claude が vault を検索・参照し、頼まれればノートを書く（CLI とスキル）

記録だけでは保管庫にしかなりません。呼び出しと再開があることで、蓄積した知識が
次の会話に戻ってきます。
...
REST API 経由の読み書きは、コマンド 1 つで実際に試せます。
...
✓ 接続       http://127.0.0.1:27123（HTTP 200）
✓ 一覧取得   vault 直下 312 件
✓ 作成       _claude-verify-1789297067404.md
✓ 読み戻し   書いた内容が一致しました
✓ 追記       末尾に追加されました
✓ 検索       作成したノートが見つかりました
✓ デイリー   今日のノートが取得できました
✓ 後片付け   _claude-verify-1789297067404.md を削除しました

REST API 経由の読み書きは正常に動いています。
```

使うのは自分で作った検証用ノート 1 枚だけで、最後に必ず消します。既存のノートには
触れません。問題があればその行と直し方が出て、終了コードが 1 になります。

#### MCP 側の確認

`verify` が確かめるのは CLI が使う REST API です。MCP 接続は別に確認します。
...
`obsidian` が接続済みと表示されれば設定は通っています。`OBSIDIAN_API_KEY` が
未設定だと「変数が見つからない」旨の警告が出ます。

**2. 実際にツールを使わせる**

接続表示だけでは、ツールが呼べるかまでは分かりません。Claude にこう頼んでください。

```
Obsidian の MCP ツールを使って、vault のノートを1件読んで、
使ったツール名とノートのパスを教えて
```

`mcp__obsidian__*` のツール名とノートの内容が返れば、MCP 経由で動いています。
「ツールが見つからない」と言われた場合は接続できていません。

**3. 失敗したときは**

`claude mcp list` でエラーが出る場合、多くは証明書です。プラグイン設定で
非暗号化 HTTP（既定 27123 番）を有効にし、`.mcp.json` の url を
`http://127.0.0.1:27123/mcp/` に変えてください（`setup` が自動で行います）。
...
プロンプトを送ると、`UserPromptSubmit` フックが vault を検索し、**関連するノートを
自動で会話に差し込みます。** こちらから「調べて」と言わなくても、蓄積した知識が
会話に戻ってきます。

- 関連度は、質問文から取り出した語がどれだけ一致するかで決まります
  （日本語は漢字・カタカナの連なりを語として扱います）
- 会話ログのフォルダは除外します。除外しないと、自分の書いたログを読み返すだけの
  堂々巡りになるためです
- 差し込む量は 3 ノート・1500 文字までに抑えています
- 関連するノートが無ければ何も差し込みません

同じ仕組みはコマンドからも使えます。

```bash
node .claude/tools/obsidian.mjs recall "海水魚の水槽でバクテリアの定着について"
```

単語で引く `search` と違い、質問文をそのまま渡せます。`--all` で会話ログも対象に含みます。

| 変数 | 既定値 | 説明 |
| --- | --- | --- |
| `OBSIDIAN_RECALL` | 有効 | `off` で自動注入を無効にする |
| `OBSIDIAN_RECALL_LIMIT` | `3` | 差し込むノート数 |
| `OBSIDIAN_RECALL_EXCLUDE` | なし | 除外するフォルダ（カンマ区切り） |

2000 ノート（約 8MB）の vault で 1 回あたり約 100ms です。

## 3. セッションの再開（vault → 会話）

セッションを始めると、`SessionStart` フックが**同じプロジェクトの前回までのやり取りを
会話に差し込みます。** 新しいセッションが白紙から始まらず、続きから入れます。

```
このプロジェクト（osakanavlog）での前回までのやり取りです。
続きから始められるよう参考にしてください。既に終わった作業を繰り返さないこと。
内容が古い可能性があるので、実際のファイルで確かめてから動くこと。

## 2026-09-11 14:00
依頼: お問い合わせフォームを追加したい
結果: 静的サイトなのでフォーム送信先が必要です。今回は mailto リンクのままにし、
Formspree を使うかは次回決めることにしました。
```

- 各セッションから**最後のやり取り**だけを取ります。結論と未決事項が残るためです
- プロジェクト名（frontmatter の `project`）で絞るので、他のプロジェクトの話は入りません
- 今のセッション自身は除外します
- 直近 2 セッション・1200 文字まで

差し込まれる内容は過去の記録であって現在のコードではありません。そのため
「実際のファイルで確かめてから動くこと」を添えています。

| 変数 | 既定値 | 説明 |
| --- | --- | --- |
| `OBSIDIAN_RESUME` | 有効 | `off` で無効にする |
| `OBSIDIAN_RESUME_LIMIT` | `2` | 遡るセッション数 |
